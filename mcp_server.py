...
Usage:
  export TELLO_IP=192.168.10.X   # IP of Tello on your local network
...
try:
    ...
    logger.error("Startup connection failed (tools will return errors): %s", e)
...
async def takeoff() -> str:
    ...
    if drone.state != DroneState.CONNECTED:
        return f"Error: drone is not connected (state={drone.state.name})."
    try:
        drone.takeoff()
...
async def get_latest_camera_frame() -> Image:
    ...
    if frame is None:
        raise ValueError("No frame available. Is the drone connected and streaming?")
